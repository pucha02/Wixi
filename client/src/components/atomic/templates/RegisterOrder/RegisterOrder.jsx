  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        "http://localhost:5001/api/orders/register-order",
        {
          method: "POST",
      ...
      if (response.ok) {
        ...
        setModal({
          ...
          title: "Помилка!",
          message:
            result.message || "Сталася помилка під час оформлення замовлення.",
        });
      }
    } catch (error) {
  ...
  return (
    <div className="register-order-block">
      ...
      <div className="category-title">
        ...
        <Link to={"/register-order"}>ОФОРМЛЕННЯ ЗАМОВЛЕННЯ</Link>
      </div>
